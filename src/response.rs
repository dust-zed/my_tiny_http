...
use std::fs::File;
use std::io::{self, Cursor, Result as IoResult};
use std::{
    io::{Read, Write},
    str::FromStr,
    time::SystemTime,
...
pub struct Response<R> {
    reader: R,
    status_code: StatusCode, //need to define StatusCode
    headers: Vec<Header>,
...
pub type ResponseBox = Response<Box<dyn Send + Read>>;
...
fn build_date_header() -> Header {
    ...
    Header::from_bytes(&b"Date"[..], &date.to_string().into_bytes()[..]).unwrap()
}
...
fn choose_transfer_encoding(
    ...
    //HTTP 1.0 does not support other encoding
    if *http_version <= (1, 0) {
    ...
    R: Read,
{
    ...
    pub fn new(
        ...
        data: R,
        data_length: Option<usize>,
        additional_headers: Option<Vec<Header>>,
    ) -> Response<R> {
    ...
    pub fn with_header<H>(mut self, h: H) -> Response<R>
    where
        H: Into<Header>,
    {
        self.add_header(h.into());
        self
    }
    ...
    pub fn raw_print<W: Write>(
        ...
        if !self.headers.iter().any(|h| h.field.equiv("Server")) {
            self.headers.insert(
                0,
                Header::from_bytes(&b"Server"[..], &b"my-tiny-http"[..]).unwrap(),
            );
        };
        ...
        let (mut reader, data_length): (Box<dyn Read>, _) =
            match (self.data_length, transfer_encoding) {
                ...
                (None, Some(TransferEncoding::Identity)) => {
                    ...
                    let l = buf.len();
                    //用 Cursor 包装 Vec<u8>，使其成为 Read 对象
